...
def image_to_workflow_details(image_name, file_type):
    ...
    #setup prompt
    system_prompt=f"""
You are an AWS Solutions Architect. You will be an image of an architecture diagram of an application or workflow. 
Based on the provided diagram your goal is to provide a detailed description of the following
...
def image_to_component_list(image_name, file_type):
    ...
    #setup prompt
    system_prompt=f"""
You are an AWS Solutions Architect analyzing an Architecture Diagram. Your task is to identify AWS services that are EXPLICITLY shown in the diagram, but ONLY if they exactly match the following approved services list:
